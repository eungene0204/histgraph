...
EVENT_SEEDS: dict[str, list[str]] = {
    # 고대 (2026-09-11). **문서명은 전부 조회로 확인했다** (넘겨주기 포함).
    # 빼야 했던 것이 여기도 있다 — '고조선의 멸망'·'평양 천도'·'웅진 천도'·
    # '포상팔국의 난'은 문서가 없고, '김헌창의 난'·'장보고의 난'은 인물
    # 문서로 넘어간다 (이자겸의 난과 같은 함정). 인물을 사건 표에 넣으면
    # 사건 노드가 된다.
    "고조선": ["고조선-연 전쟁", "고조선-한 전쟁"],
    # 3~4세기 다섯은 2026-09-12 에 더했다 (연표의 214~371년이 통째로 비어
    # 있다는 지적). 이 시대의 위키백과는 사건 문서가 드물어 **분류를 훑어
    # 찾았다** — 분류:고구려가 참전한 전쟁·분류:242년 한국 …. '발기의 난'은
    # 인물 문서('발기 (고구려)')로 넘어가 뺐다 (이자겸의 난과 같은 함정).
    "고구려": [
        "고구려-수 전쟁", "살수 대첩", "고구려-당 전쟁", "안시성 전투",
        "주필산 전투", "좌원 전투", "막리지의 난", "고구려 부흥운동",
        "고구려-위 전쟁", "공손연의 난", "고구려-백제 전쟁",
        "광개토대왕의 전쟁", "고구려-왜 전쟁",
    ],
    "백제": [
        "황산벌 전투", "관산성 전투", "백강 전투", "백제 부흥운동",
        "백제의 멸망", "백제-북위 전쟁",
    ],
    "신라": [
        "나당 전쟁", "매소성 전투", "기벌포 전투", "비담의 난",
        "원종·애노의 난", "나제동맹", "신라의 삼국통일",
    ],
    "발해": ["천문령 전투"],
    ...
    "고려": [
        # 후삼국 통일
        "고창 전투", "공산 동수 전투", "일리천 전투", "황산 전투 (936년)",
        "왕규의 난",
        # 거란
...
ORG_SEEDS: dict[str, list[str]] = {
    # 고대의 나라들. **이 표가 없으면 시대가 이름을 갖지 못한다** — 부여·
    # 옥저·동예·삼한은 Wikidata 인물이 거의 없어(부여 14 · 마한 2) 사람을
    # 타고 들어오지 못하고, 한군현은 아예 걸릴 자리가 없다.
    "고조선": [
        "단군조선", "기자조선", "위만조선",
        # 한사군은 고조선이 무너진 자리에 선 것이라 그 시대의 뒤끝으로 읽는다.
        "한사군", "낙랑군", "대방군", "현도군", "임둔군", "진번군",
    ],
    "원삼국 시대": [
        "부여", "동부여", "북부여", "옥저", "동예",
        "삼한", "마한", "진한", "변한", "진국",
    ],
    # 고구려·백제·신라의 기구. 당이 세운 도호부·도독부도 여기 든다 —
    # 맞선 쪽만 넣으면 무엇에 맞섰는지가 없다 (일제강점기의 총독부와 같다).
    "고구려": ["태학", "경당", "안동도호부", "보덕국"],
    "백제": ["웅진도독부"],
    "신라": ["화랑", "9서당", "10정", "계림주도독부", "우산국"],
    "발해": ["주자감"],
    "후삼국 시대": ["후백제", "태봉"],
...
CONCEPT_SEEDS: dict[str, list[str]] = {
    # 고대는 **제도로 적힌 시대**다. 남은 사건이 전쟁뿐이라, 제도를 빼면
    # 그 천 년이 '누가 누구를 쳤다'만 남는다 (고려에 전시과·음서를 넣은
    # 것과 같은 이유다).
    "고조선": ["팔조지교", "단군 신화"],
    "원삼국 시대": ["소도 (한국사)"],
    "고구려": ["진대법", "제가", "서옥제"],
    "백제": ["정사암"],
    "신라": [
        "골품제", "화백 (신라)", "독서삼품과", "녹읍", "관료전",
        "상수리 제도", "신라의 행정 구역", "촌주", "신라장적",
    ],
    "후삼국 시대": ["호족", "구산문", "풍수"],
...
MONARCH_SEEDS: dict[str, list[tuple[str, str]]] = {
    # 고대의 임금 145명 (2026-09-11). **라벨은 대개 문서명 그대로다** —
    # 삼국의 시호는 묘호와 달리 거의 겹치지 않아서, 고려·조선처럼 왕조를
    # 앞에 붙이면 읽기만 길어진다. 겹치는 것만 가른다: 성왕·혜왕·무왕은
    # 백제와 발해에 각각 있고(위키백과도 괄호로 가른다), 고구려 태조대왕은
    # 고려·조선 태조와 헷갈리며, 신라 선덕왕(37대)은 선덕여왕(27대)과
    # 다른 사람이다.
    #
    # 재위 날짜가 여기서 오는 것이 특히 중요하다 — 고대 임금은 Wikidata 의
    # P39 문장이 거의 없어 `reigns` 가 채우지 못한다. 연표 왼쪽의 띠가
    # 고려 918년에서 시작해 그 앞이 통째로 비어 있던 까닭이다.
    "고구려": [
        ("동명성왕", "동명성왕"),
        ("유리명왕", "유리명왕"),
        ("대무신왕", "대무신왕"),
    ...
    # 가야는 금관가야(수로왕~구형왕)와 대가야(이진아시왕~도설지왕)를 함께
    # 담는다. 둘은 다른 나라지만 우리 그래프의 시대는 '가야' 하나다.
    "가야": [
        ("수로왕", "수로왕"),
        ("거등왕", "거등왕"),
        ("마품왕", "마품왕"),
    ...
    # 후삼국의 셋은 시호가 아니라 이름이다. 왕건은 고려 태조라 그쪽에 있다.
    # **나라로 가른다** — 후삼국은 시대이지 나라가 아니라서, 한 이름으로
    # 묶으면 `positions` 가 '후삼국 시대 왕'이라는 없던 자리를 만든다.
    "후백제": [
        ("견훤", "견훤"),
        ("신검", "신검"),
    ],
