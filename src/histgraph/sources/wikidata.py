...
POLITIES = {
    # 고대 (2026-09-11). 실측한 인물 수가 적다 — 고조선 6 · 위만조선 8 ·
    # 부여 14 · 가야 14 · 통일신라 9 · 후백제 5 · 태봉 5 · 마한 2. 그래도
    # 넣는 까닭은 이 이름이 **씨앗의 열쇠**이기 때문이다: `props.polity` 가
    # 없으면 `scope` 가 그 시대의 인물을 하나도 못 고른다.
    "Q28405": "고조선",
    "Q703340": "위만조선",
    "Q28511": "부여",
    "Q704278": "마한",
    ...
    "Q715210": "금관가야",
    "Q1157054": "대가야",
    # 통일신라는 신라와 다른 항목이라 P27/P17 이 갈려 적힌다. 신라 시대가
    # 이 이름을 함께 담는다 (`scope.ERAS['silla']`).
    "Q715257": "통일신라",
    "Q698239": "후백제",
    "Q699871": "태봉",
    "Q28370": "고구려",
...
WD_CLASS_TO_TYPE: dict[str, str] = {
    ...
    "Q11514315": "period",
    # 연호는 사건이 아니라 **해의 이름**이다 (Q698350=한국의 연호, 라벨 대조로
    # 확인). 실측: 태봉·후백제의 무태·성책·수덕만세·정개가 사건 노드로 앉아
    # 904·905·911·915년 연표를 차지하고 있었다 — "911년에 일어난 일: 수덕만세".
    "Q698350": "period",
    "Q16261338": "org",
    # 훈장·상은 사건이 아니다. 실측: 태극장·자응장·팔괘장·서봉장·금척대수장이
    # 사건 노드로 앉아 있다가 연대까지 채워지자 1900~1904년 연표를 차지했다 —
    # "1900년에 일어난 일: 대훈위 금척대수장". 제도이므로 개념으로 간다.
    # (Q56291528=state order · Q193622=훈장 · Q618779=상 · Q369359=여성 훈장,
    #  전부 라벨 대조로 확인)
    "Q56291528": "concept", "Q193622": "concept",
    "Q618779": "concept", "Q369359": "concept",
    # 제도는 사건이 아니다. 실측(2026-09-11): 팔조지교·독서삼품과·관료전·
    # 상수리 제도가 사건 노드로 앉아 고조선·신라의 연표에 서 있었다.
    # (Q7748=법 · Q2072146=인사 선발 · Q1165944=토지 이용 · Q16188712=인질 정책,
    #  전부 라벨 대조로 확인)
    "Q7748": "concept", "Q2072146": "concept",
    "Q1165944": "concept", "Q16188712": "concept",
    # Q190928=조선소. '대한조선공사'가 사건으로 앉아 있었다.
    "Q190928": "org",
...
def spans_from_rows(rows: list[dict]) -> dict[str, tuple[str | None, str | None]]:
    ...
    **P571(설립)을 먼저 본다.** 왕조·조직에는 설립/해체가 맞는 술어다.
    ...
    for r in rows:
        ...
        # **기원전은 문자열로 비교하면 뒤집힌다** — '-0107'(기원전 108년)이
        # '-2332'(기원전 2333년)보다 사전 순으로 앞선다. 그래서 고조선과
        # 위만조선의 존속 기간이 '끝이 시작보다 앞선다'로 버려지고 있었다
        # (2026-09-11). 해로 견준다.
        if start and end and _year_num(end) < _year_num(start):
            continue
        if not (start or end):
            continue
