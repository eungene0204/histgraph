...
//
// **'관련'은 방향을 말하지 않아서 바꿨다** (2026-09-06 사용자 지적: "'이
// 인물이 만든 것' 이런 식으로 바꿨으면 좋겠는데"). 이 나무는 이 노드에서
// 뻗어 나간 쪽이므로 방향이 있는 말이어야 한다.
//
// '만든 것'을 그대로 쓰지 않은 이유 둘. 만들다는 제작에 가까워 역사 인과에
// 덜 맞고, **원인 쪽은 인물에게 아예 뜨지 않는다** — 인과 엣지 2,350건 중
// 인물이 결과인 것이 0건이다(사람은 원인만 될 수 있게 막아 두었다). 그래서
// '이 인물을 만든 것'은 설 자리가 없다.
//
// '일으킨 일'이 가장 자연스러운 한국어지만(난을 일으키다) 이 나무는 네
// 걸음까지 따라간 것이라, 먼 것까지 한 사람이 일으켰다고 단정하게 된다.
// '비롯된'은 방향을 말하면서 그 단정을 피한다.
export function chainHeads(type) {
  const noun = CHAIN_NOUN[type];
  if (!noun) return ['이 일을 부른 것', '이 일이 부른 것'];
  return [`이 ${noun}에 앞선 일`, `이 ${noun}에서 비롯된 일`];
}
