...
// 작품을 만든 방식(`creators.ROLES`)별 문장. 엣지 라벨이 방식이다.
// 타입 이름 하나로 부르면 "정선이 인왕제색도를 만들었다"가 된다 — 그린 것과
// 쓴 것과 지은 것과 엮은 것은 다른 일이다. 판정은 사람이 표에 적는다
// (`data/creators.tsv`) — 초상의 주인공도, 간행을 명한 임금도 만든 사람이 아니다.
export const CREATED_SENTENCE = {
  '그림': (a, b) => `${a}${pt(a, '이', '가')} ${b}${pt(b, '을', '를')} 그렸다`,
  // 서예·비문의 서자·사경. 작품 전체가 아니라 그 글씨를 쓴 사람이다.
  '글씨': (a, b) => `${a}${pt(a, '이', '가')} ${b}의 글씨를 썼다`,
  '저술': (a, b) => `${a}${pt(a, '이', '가')} ${b}${pt(b, '을', '를')} 지었다`,
  '편찬': (a, b) => `${a}${pt(a, '이', '가')} ${b}${pt(b, '을', '를')} 엮었다`,
  '제작': (a, b) => `${a}${pt(a, '이', '가')} ${b}${pt(b, '을', '를')} 만들었다`,
  // 시주·발원·의뢰. 손으로 만든 사람이 아니라 만들게 한 사람이다.
  '발원': (a, b) => `${a}${pt(a, '이', '가')} ${b}${pt(b, '을', '를')} 만들게 했다`,
};
...
export const SENTENCE = {
  ...
  // 만든 방식이 적혀 있으면 그것이 타입 이름을 이긴다 (`creators.ROLES`).
  // 라벨이 없는 옛 엣지(산문 추출 17건)는 '만들었다'로 남는다.
  created: (a, b, o = {}) => (CREATED_SENTENCE[o.label] || CREATED_SENTENCE['제작'])(a, b),
  ...
  related_to: (a, b, o) => (o.label === '다음'
    ...
    // 스키마가 안 맞아 낮춰진 제작 관계도 라벨이 방식을 들고 있다
...
export const LABEL_DIR_HEAD = {
  ...
  // 작품을 만든 방식 여섯 (`creators.ROLES`). 사람 쪽(out)에서는 그가 만든
  // 것들의 목록이고, 작품 쪽(in)에서는 만든 사람이다 — '제작 · 12' 로는
  // 무엇의 목록인지 알 수 없고, 김정희의 상세에 '제작'이라고 서면 그가
  // 세한도를 그렸는지 글씨를 썼는지 지었는지가 사라진다.
