...
/**
 * 계정을 읽고 난 뒤, **브라우저에 남은 것을 쓸지 말지** 정한다.
 *
 * 여기가 갈라야 하는 것들은 겉모습이 같다 — 전부 '계정은 비었고 브라우저에는
 * 있다' 이다. **가르는 것은 주인 표이고, 없는 것과 빈 것은 다른 뜻이다.**
 *
 * | 표 | 뜻 | 어떻게 |
 * |---|---|---|
 * | `''` (빈 표) | 로그인하기 전에 적은 글 | 이어받아 계정에 올린다 |
 * | 내 표 | 계정에 올라갔다가 **지워진** 글 | 사본도 지운다 |
 * | 없음(`null`) | 표가 생기기 전(2026-09-11 이전)에 남은 사본 | 사본도 지운다 |
 * | 남의 표 | 앞사람이 쓰고 간 것 | `readLife` 가 지운다 |
 *
 * **표가 없는 것을 '로그인 전에 적은 글'로 읽으면 안 된다** — 2026-09-11 에
 * 그렇게 읽어서, 계정에서 지운 내 역사가 새로고침마다 되살아나 계정으로 다시
 * 올라갔다 ("아직도 잘 보여"). 로그인 전에 적은 글에는 **빈 표가 찍힌다**
 * (`writeLife(doc, '')`). 표가 아예 없다는 것은 로그인한 채로 적어 계정에
 * 올렸던 옛 사본이라는 뜻이고, 그 계정이 비었다면 지운 것이다.
 *
 * `accountRead` 가 거짓이면(네트워크로 계정을 못 읽었다) **아무 판단도 하지
 * 않는다** — 못 읽은 것을 '지웠다'로 읽으면 멀쩡한 자료를 지운다.
 */
export function localAfterAccount({ owner = '', accountRead = false, accountHasDoc = false } = {}, store) {
  if (owner && accountRead && !accountHasDoc && markedOwner(store) !== "") {
    forgetLife(store);
    return null;
  }
  return readLife(owner, store);
}
