 ...
 *
 * 여기가 갈라야 하는 두 가지는 겉모습이 같다 — 둘 다 '계정은 비었고 브라우저에는
 * 있다' 이다.
 *
 * 1. **로그인 전에 적은 글.** 주인 표가 없다(`''`). 그 사람이 로그인했으니
 *    이어받아 계정에 올린다 — 이것이 없으면 로그인하는 순간 적어 둔 것이 사라진다.
 * 2. **계정에서 지운 글.** 내 표가 찍혀 있는데 계정에는 없다. 한 번 계정에
 *    올라갔다가 지워졌다는 뜻이므로 **사본도 따라 지운다.**
 *
 * 2026-09-11 에 2번이 없어서, 계정에서 지운 내 역사가 화면을 새로 고치자마자
 * 브라우저 사본에서 되살아나 계정으로 다시 올라갔다 ("아직 내 역사가 보이는데?").
 *
...
export function localAfterAccount({ owner = '', accountRead = false, accountHasDoc = false } = {}, store) {
  if (owner && accountRead && !accountHasDoc && markedOwner(store) === owner) {
    forgetLife(store);
    return null;
  }
